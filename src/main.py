...
REPOSITORY_URL = "https://gitlab.com/mrollet/game-stats-manager"
VERSION = 1.0
YEARS = '2015, 2016'
...
DEFAULT_TS = 1
DEFAULT_CATEGORIES = [("No categories, edit yours or select a predefined game", False)]

sports = []
sports.append({'name': 'Basketball', 'nbPlayers': 10, 'timeSplit': 4,
              'categories': [("2pts shot", True), ("3pts shot", True), ("Free Throw", True), 
                             ("Turnover", False), ("Assist", False), ("Off Rebounds", False),
                             ("Def Rebounds", False), ("Steal", False), ("Block", False)]})

sports.append({'name': 'Soccer', 'nbPlayers': 11, 'timeSplit':2, 
               'categories': [("Goal", True), ("Tackling", True), ("Dribbling", True), 
                             ("Assist", False), ("Receiving", False),
                             ("Steal", False), ("Block", False)]})
...
class Header:
    ...
    def __init__(self, categories):
        ...
        if categories:
            ...
            c = Categorie("No categories, edit yours or select a predefined game")
            self.cat.append(c)
...
class Game:
    ...
    def chgNbPlayers(self, root):
        ...
        """    
        msg = ("Warning : this action will reset scores\n\n"
            "Enter the number of players to display : ")
        newNb = simpledialog.askinteger("Number of Players", msg, minvalue=1, maxvalue=MAX_NB_PLAYERS)
        if newNb:
    ...
    def chgNbTs(self, root):
        ...
        """
        msg = ("Warning : this action will reset scores\n\n"
            "Enter the number of time divisions to consider : ")
        newTs = simpledialog.askinteger("Number of time divisions", msg, minvalue=1, maxvalue=MAX_TS)
        if newTs:
...
def save_template(root, game):
    ...
    name = ''
    while len(name) < 1:
        name = simpledialog.askstring('Game template name',
                                  'Please enter a name for this game template (ex : "Basketball", "Hockey", ...)')
        if name is None: 
            # askstring dialog returns None if cancelled
            return 1
        
        if len(name) < 1:
            messagebox.showinfo(title='Empty entry', message='Please enter a name')
        
    if os.path.exists(SAVE_TEMP_PATH + '/{}.gsm'.format(name)):
        replace = messagebox.askokcancel(title='Warning', icon='warning', default='cancel',
                                         message='A game template with this name already exists. Proceeding will replace it.')
        if not replace:
            return 1
    try:
...
def ManageTemplates(root, game):
    def rename(root, game, old, window):
        ...
        new = ''
        while len(new) < 1:
            new = simpledialog.askstring('New name', 'Enter a new name for this template')
            if new is None:
                return 1
    ...
    def remove(root, game, filename, labelframe, lfchildren):
        ...
        Label(labelframe, text='Removed', justify=CENTER).grid(padx=100, pady=20)
    ...
    if gsmfiles:
        for i, filename in enumerate(gsmfiles):
            ...
            try:
                ...
                l = Label(lf, text='No preview found\nLoad the game template and save it to generate one')
                
            btnframe = Frame(lf)
            rmbtn = Button(btnframe, text="Remove", command=lambda lf=lf, f=filename, l=[l, btnframe]: remove(root, game, f, lf, l))
            editbtn = Button(btnframe, text="Rename", command=lambda path=path, old=filename: rename(root, game, old, window))
            editbtn.grid(row=0, column=0)
            rmbtn.grid(row=0, column=1)
            l.grid(row=0, column=0)
            btnframe.grid(row=1, column=0)
    else:
        Label(frame, text=("There is no templates to manage.\nTo create templates, configure a game "
                            "via the 'Edit' menu and save it as a game template via the 'File' menu.")).grid()
...
class chgHeaderWindow(Toplevel):
    ...
    def __init__(self, game, root):
        ...
        self.title("Columns headers")
        self.warning = Label(self, text="Warning : this action will cause the score table to reset.")
        self.text = Label(self, text="Feel free to Add, Edit, or Remove sections.")
        self.btn1 = Button(self, text="Cancel", command=self.destroy)
...
class CatConfigField(Frame):
    ...
    def __init__(self, master, cat, dual, i):
        ...
        self.addbtn = Button(self, text='Add', command=lambda master=master, i=i+1: self.addfield(master, i))
        if i > 0: # Doesn't concern the 'Player' field
            self.rmbtn = Button(self, text='Remove', command=lambda master=master: self.rmfield(master))
            self.chkvar = IntVar()
...
class FileMenu(Menubutton):
    def __init__(self, master, root, game):
        Menubutton.__init__(self, master, text="File")
        ...
        self.newmenu.add_command(label="Load a custom game template...", command=lambda root=root, game=game: load_game(root, game, 'template'))
        self.newmenu.add_separator()
        self.newmenu.add_command(label="Reset this game", command=game.reset_game)
        self.menu.add_cascade(label="New game", menu=self.newmenu)
        self.menu.add_command(label="Open a game...", command=lambda root=root, game=game: load_game(root, game, 'game'))
        self.menu.add_separator()
        
        self.menu.add_command(label="Save game as...", command=lambda game=game: save(game, 'game'))
        self.menu.add_command(label="Save stats as...", command=lambda game=game: save(game, 'stats'))
        self.menu.add_separator()
        
        self.templatemenu = Menu(self, tearoff=0)
        self.templatemenu.add_command(label="Save the current game template...", command=lambda game=game: save_template(root, game))
        self.templatemenu.add_command(label="Manage...", command=lambda root=root, game=game: ManageTemplates(root, game))
        self.menu.add_cascade(label="Templates", menu=self.templatemenu)
        
        self.menu.add_command(label="Generate stats", command=lambda root=root, game=game: show_stats(root, game))
        self.menu.add_separator()
        
        self.menu.add_command(label="Exit", command=lambda root=root, game=game: safeExit(root, game))

class EditMenu(Menubutton):
    def __init__(self, master, root, game, frame):
        Menubutton.__init__(self, master, text="Edit")
        self.menu = Menu(self, tearoff=0)
        self["menu"] = self.menu
        self.menu.add_command(label="Categories", command=lambda game=game, root=root: game.chgHeader(root))
        self.menu.add_command(label="Number of players", command=lambda root=root, game=game: game.chgNbPlayers(root))
        self.menu.add_command(label="Number of time divisions", command=lambda root=root, game=game: game.chgNbTs(root))

class DisplayMenu(Menubutton):
    def __init__(self, master, root):
        Menubutton.__init__(self, master, text="Display")
        self.menu = Menu(self, tearoff=0)
        self["menu"] = self.menu
        self.menu.add_checkbutton(label="Fullscreen", variable=root.fullscreen, command=root.toogle_fullscreen, accelerator="F11")
...
class HelpMenu(Menubutton):
    def __init__(self, master):
        Menubutton.__init__(self, master, text="Help")
        self.menu = Menu(self, tearoff=0)
        self["menu"] = self.menu
...
class TsFrame(Frame):
    def __init__(self, master, game):
        ...
        self.EOMbtn = Button(self, text='End of Match', 
...
class MainMenu(Frame):
    ...
    def __init__(self, root, game):
        ...
        self.teamEntry = TeamEntry(self, 'Team : ', game.team)
        self.opponentEntry = TeamEntry(self, 'Opponent : ', game.opponent)
...
class MenuStats(Menu):
    def __init__(self, statsWin, game):
        ...
        self.filemenu.add_command(label="Save stats", command=lambda game=game: save(game, 'stats'))
        self.filemenu.add_separator()
        self.filemenu.add_command(label="Exit", command=statsWin.destroy)
